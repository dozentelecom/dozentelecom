...
import CustomerAccountControls from "@/components/admin/CustomerAccountControls";
...
export default async function CustomerDetailsPage({
  ...
  const funding: any[] = await Funding.find({
    userId: user._id,
  })
    .sort({ createdAt: -1 })
    .limit(100)
  ...
  const totalProfitKobo = transactions.reduce(
    (sum, tx) =>
      sum + Number(tx.profitKobo || 0),
    0
  );

  const totalFundingKobo = funding.reduce(
    (sum, item) =>
      sum + Number(item.creditKobo || 0),
    0
  );
      ...
      <main className="admin-dashboard-content">
        ...
        <div className="admin-customer-overview-grid">
          ...
          <div className="card">
            ...
            <div
              className={`admin-customer-vip ${String(
                vipLevel
              ).toLowerCase()}`}
              style={{
                marginTop: 10,
